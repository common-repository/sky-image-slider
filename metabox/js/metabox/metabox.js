jQuery(document).ready(function($){
	...
	var $all_images = $('#nsr_images_container ul.images_list');
	var $isstyle = 'style="display:block;width:100%"';

	jQuery('.nsr_add_images').on( 'click', function( event ) {
       ...
       // Create the media frame.
		...
		// When an image is selected, run a callback.
         images_frame.on( 'select', function() {
					 	...
					 	$('.noimgs').remove();
							...
							if ( typeof attachment.sizes.thumbnail == "undefined" ) {
                             ...
                             $all_images.append('\
								...
								<input type="hidden" name="nsr_meta[nsr_meta_select_images][' + attachment.id + '][images]" value="' + attachment.id + '" />\
								<div class="ewic-shorters">\
                                <img src="' + attachment_img + '" />\
								<span class="ewic-del-images"></span>\
								<label for="title-for-'+ attachment.id +'"> Caption </label>\
								<input class="images-title" type="text" name="nsr_meta[nsr_meta_select_images][' + attachment.id + '][ttl]" value="' + attachment.title + '"/></div>\
                                </li>').hide().fadeIn(300);
            ...
            // Remove images
            $('#nsr_images_container').on( 'click', '.ewic-del-images', function() {
				
				jQuery(this).parent().fadeOut(500, function() { 
				
					$(this).closest('li.ewicthumbhandler').remove();
					
						nsr_is_img_exist(jQuery('.ewic-del-images'));
		...
		function nsr_is_img_exist(sel) {
			
			if ( !$(sel).length ) {
				
				dat = {};
				dat['action'] = 'nsr_img_remove';
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
              ...
              {
                element: '#nsr_images_container',
                intro: "All selected images will listed here.",
                position: 'right'
              },
              {
                element: '#nsr_meta_settings',
                intro: 'Finally, you can adjust the options below to fit your needs.',
                position: 'left'
              },
              {
                element: '#publish',
                intro: "When you are done, you can save the slider and put the slider into your post/page using Shortcode.",
                position: 'bottom'
              },
             {
                element: '#ewicbuydiv',
